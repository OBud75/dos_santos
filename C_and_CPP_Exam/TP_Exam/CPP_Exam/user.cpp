...
void User::save() {
    ...
    // Le fait d'éviter la copie ne devrait pas être le but premier d'un pointeur partagé mais
    // plutôt le fait de déléguer la gestion de la mémoire au conteneur (ici unordered_map)
    // Si le but est uniquement d'éviter la copie, on peut par exemple utiliser std::move
    // ou même un pointeur ou une référence "classique".
    // Pas de commentaire > un mauvais commentaire
}
